...
class Scene:
    ...
    def build_robot(self, numVertebrae, vertebraeRadius, numLegs, numLegSegments, legLength):
        ...
        curY = absY
        vertebraeWidth = vertebraeRadius * 2  # Width of each vertebra
        vertebraeHeight = vertebraeRadius * 2  # Height of each vertebra

        # Add vertebrae
        for i in range(numVertebrae):
            self.add_rect(curX, curY, vertebraeWidth, vertebraeHeight, i - 1)
            curX += vertebraeWidth  # Move to the next vertebra position

        totalVertebraeWidth = curX  # Total width of all vertebrae
