...
def robot(scene, num_vertebrae, num_legs, num_leg_segments, leg_length):
    scene.set_offset(0.1, 0.03)
    scene.build_robot(num_vertebrae, 0.04, num_legs, num_leg_segments, leg_length)
...
def main(num_vertebrae, num_legs, num_leg_segments, leg_length, iters=100):
    # Initialization
    scene = Scene()
    # scene.build_robot(num_vertebrae, 0.04, num_legs, num_leg_segments, leg_length)
    robot(scene, num_vertebrae, num_legs, num_leg_segments, leg_length)
    scene.finalize()
    allocate_fields()
...
if __name__ == '__main__':
    import sys
    if len(sys.argv) == 5:
        num_vertebrae = int(sys.argv[1])
        num_legs = int(sys.argv[2])
        num_leg_segments = int(sys.argv[3])
        leg_length = float(sys.argv[4])
        losses = main(num_vertebrae, num_legs, num_leg_segments, leg_length)
        print(losses)
    else:
        print("Usage: python diffmpmEvoTest.py <num_vertebrae> <num_legs> <num_leg_segments> <leg_length>")
