...
class Scene:
    ...
    def build_robot(self, string):
        sum = 0
        for i in range(len(string)):
            sum += ord(string[i])
        num_shapes = (sum % 5) + 3  # Generates between 3 and 7 shapes
        base_size = ((sum % 10) + 5) * 0.05  # Generates a base size between 5*0.05 and 14*0.05
        current_x = 0
        current_y = 0
        for i in range(num_shapes):
            ...
            if num < 0.5:
                # Rectangle
                height = base_size * (1 + (i % 3)) * 0.1  # Vary height based on index
                width = base_size * 0.1  # Fixed width for rectangles
                self.add_rect(current_x, current_y, width, height, i - 1, ptype=1)
                # Update position for the next shape
                current_x += (width * 0.9)
                last_height = (height * 0.9)
            else:
                # Circle
                radius = base_size * 0.05 * (1 + (i % 2))  # Vary radius based on index
                self.add_circle(current_x, current_y, radius, i - 1, ptype=1)
                # Update position for the next shape
                current_x += (2 * radius) * 0.9  # Move by the diameter of the circle
                last_height = (2 * radius) * 0.9  # Track the height of the last shape (diameter)
            # Add vertical spacing every 2 shapes
            if i > 0 and i % 2 == 1:  # After every pair of shapes 
                current_y += last_height  # Move down by the height of the last shape
                current_x = 0  # Reset x-position for the next row
        self.set_n_actuators(num_shapes - 1)
        return
...
def robot(scene):
    ...
    # scene.build_robot('I Love Artificial Life!')
    scene.build_robot('Hello!')
